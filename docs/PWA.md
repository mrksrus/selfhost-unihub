...
Run `node --test api/tests/notifications.test.js api/tests/notification-worker.test.js`. The optional `notifications-mysql-integration.test.js` runs when `MYSQL_TEST_HOST` and the usual `MYSQL_TEST_*` settings are provided. It uses connection-local temporary tables to test real queries and transactions without altering the configured database. MySQL does not support foreign keys on temporary tables, so production foreign-key DDL also needs an API startup check against a disposable database.
